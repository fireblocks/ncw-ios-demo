    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                List(viewModel.items, id: \.assetSummary.id) { item in
                                ...
                                HStack(spacing: 8) {
                                    ...
                                    if let data = item.keyData?.data {
                                        ...
                                        Image(uiImage: item.isExposed ?  AssetsIcons.eye.getIcon() : AssetsIcons.eyeCrossedOut.getIcon())
                                            ...
                                            .onTapGesture {
                                                withAnimation {
                                                    if let index = viewModel.items.firstIndex(where: {$0.assetSummary.asset?.id == asset.id}) {
                                                        viewModel.items[index].isExposed.toggle()
                                                    }
                                                }
                                            }
                                    }
                                    ...
                                    HStack(spacing: 8) {
                                        ...
                                        Image(uiImage: item.isExposed ?  AssetsIcons.eye.getIcon() : AssetsIcons.eyeCrossedOut.getIcon())
                                            ...
                                            .onTapGesture {
                                                withAnimation {
                                                    if let index = viewModel.items.firstIndex(where: {$0.assetSummary.asset?.id == asset.id}) {
                                                        viewModel.items[index].isWifExposed.toggle()
                                                    }
                                                }
                                            }
                                    }
...
#Preview {
    DeriveKeysView(viewModel: DeriveKeysView.ViewModel(privateKey: "XXXX"))
}
